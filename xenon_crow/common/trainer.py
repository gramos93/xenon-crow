...
class ReinforceTrainer(object):
    ...
    def generate_episode(cls, env, agent):
        ...
        while not terminated:
            action, log_prob, value = agent.get_action(tensor(state, dtype=float32).unsqueeze(0))
            next_state, r, done, trunc, *_ = env.step(action)
            terminated = done or trunc

            agent.replay_buffer.push((log_prob, r, value))
            reward += r
    ...
    def run(env, agent, max_episodes):
        ...
        for _ in progress_bar:
            episode_rewards.append(
                ReinforceTrainer.generate_episode(env, agent)
            )
            agent.update()
